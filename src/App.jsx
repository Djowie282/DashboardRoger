...
// Twelve Data symbol overrides (symbols that need exchange suffix)
const TD_MAP = { SIVE:"SIVE:XSTO", VWCE:"VWCE:XETRA", WEBN:"WEBN:IBIS2" };

// ─── Default data ─────────────────────────────────────────────────
const EXAMPLE = {
  snapshot:"1 jan 2025 · Voorbeeld",
  ...
  positions:[
    ...
    {id:2,sym:"NVDA",name:"NVIDIA Corp.",       qty:20,  px:875.00,cost:9000, ccy:"USD",earnings:"",pt:1100,thesis:"AI infra pick.",td_sym:""},
    {id:3,sym:"MSFT",name:"Microsoft Corp.",    qty:30,  px:415.00,cost:10500,ccy:"USD",earnings:"",pt:480,thesis:"Cloud + AI moat.",td_sym:""},
  ],
  realized:[
    {id:1,sym:"AMZN",desc:"Amazon deels verkocht",pnl:1200},
    {id:2,sym:"META",desc:"Meta te vroeg verkocht",pnl:-400},
  ],
  watchlist:[
    {id:1,sym:"TSM",name:"Taiwan Semiconductor",px:160,ccy:"USD",pt:210,thesis:"Beste fab ter wereld."},
  ],
  deposits:[
    {id:1,date:"2024-01-01",amount:10000,desc:"Eerste storting"},
    {id:2,date:"2024-06-01",amount:5000, desc:"Bijstorting"},
  ],
  history:[],
};
...
const fmtN = (n,d=0) => Math.abs(n).toLocaleString("nl-NL",{minimumFractionDigits:d,maximumFractionDigits:d});
const fmtM = (sym,n,d=0) => (n<0?"−":"")  +sym+fmtN(n,d);
const fmtS = (sym,n,d=0) => (n<0?"−":"+")+  sym+fmtN(n,d);
const fmtP = (n,d=1) => (n>=0?"+":"")+n.toFixed(d)+"%";
const fmtDateShort = d => { if(!d)return""; const dt=new Date(d); return dt.toLocaleDateString("nl-NL",{day:"numeric",month:"short",year:"2-digit"}); };
...
const enrich = (positions,fx) =>
  positions.map(p=>{
    ...
    return {...p,value:val,upnl:r2(val-p.cost),pxEUR:r2(p.px*r)};
  }).sort((a,b)=>b.value-a.value);

const daysUntil = d => d?Math.ceil((new Date(d)-new Date())/86400000):null;
const fmtEarnings = d => d?new Date(d).toLocaleDateString("nl-NL",{day:"numeric",month:"short"}):null;
...
// ─── Price fetch (Twelve Data) ─────────────────────────────────────
async function fetchPrices(apiKey, positions, currency) {
  if (!apiKey?.trim()) throw new Error("Geen API key — voeg toe in Instellingen");

  const posSyms  = positions.map(p => TD_MAP[p.sym] || p.sym);
  const fxSyms   = ["EUR/USD","EUR/SEK"];
  const allSyms  = [...new Set([...posSyms,...fxSyms])];

  const url = `https://api.twelvedata.com/price?symbol=${allSyms.join(",")}&apikey=${apiKey.trim()}`;
  ...
  for (const [sym, val] of Object.entries(data)) {
    ...
    if (sym === "EUR/USD") {
      newFx.usd = r2(1 / price); // EUR/USD rate → convert to "EUR per 1 USD"
    } else if (sym === "EUR/SEK") {
      newFx.sek = parseFloat((1 / price).toFixed(6));
    } else {
      // Map back to original symbol
      const orig = Object.entries(TD_MAP).find(([,v]) => v === sym)?.[0] || sym;
      newPx[orig] = price;
    }
  }

  return { prices: newPx, fx: newFx };
}
...
// Price in native currency + EUR equivalent in brackets
const PriceDisplay = ({px, ccy, pxEUR, sym: cursym}) => {
  if (ccy === "EUR") return <span style={{fontFamily:MONO}}>{cursym}{px.toFixed(2)}</span>;
  if (ccy === "SEK") return (
    <span style={{fontFamily:MONO}}>
      SEK {px.toFixed(2)}
      <span style={{fontSize:10,color:DIM}}> (≈{cursym}{pxEUR.toFixed(2)})</span>
    </span>
  );
  // USD
  return (
...
function EarningsCalendar({positions}) {
  const upcoming = positions
    .map(p=>({sym:p.sym,date:p.earnings,days:daysUntil(p.earnings)}))
    .filter(p=>p.days!==null&&p.days>=0&&p.days<=90)
    .sort((a,b)=>a.days-b.days);
  return (
    <div style={{...sCard,padding:"16px 18px"}}>
      <div style={{fontSize:10,color:DIM,textTransform:"uppercase",letterSpacing:".12em",marginBottom:12}}>Earnings Kalender</div>
      {!upcoming.length
        ? <div style={{fontSize:12,color:DIM,fontStyle:"italic"}}>Geen earnings binnen 90 dagen</div>
        : upcoming.map((e,i)=>{
          const c=ecColor(e.days);
          return (
            <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"6px 0",borderBottom:i<upcoming.length-1?`1px solid ${LINE}`:"none"}}>
              <div style={{width:36,height:36,borderRadius:8,background:`${c}18`,border:`1px solid ${c}40`,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",flexShrink:0}}>
                ...
                <span style={{fontSize:8,color:c,textTransform:"uppercase",letterSpacing:".06em"}}>dgn</span>
              </div>
              <div>
                <div style={{fontWeight:700,fontSize:13}}>{e.sym}</div>
                <div style={{fontSize:11,color:DIM}}>{fmtEarnings(e.date)}</div>
              </div>
              {e.days<=7&&<span style={{marginLeft:"auto",fontSize:10,fontWeight:700,color:RED,textTransform:"uppercase"}}>Binnenkort</span>}
            </div>
          );
        })
      }
...
function NavChart({history}) {
  if (!history?.length||history.length<2) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:180,color:DIM,fontSize:12,flexDirection:"column",gap:6}}>
      ...
      <span>Sla meerdere updates op voor de curve</span>
    </div>
  );
  ...
  return (
    <ResponsiveContainer width="100%" height={180}>
      <AreaChart data={data} margin={{top:8,right:8,left:0,bottom:0}}>
        ...
        <Tooltip contentStyle={{background:"#1a1c22",border:`1px solid ${LINE}`,borderRadius:8,fontFamily:MONO,fontSize:12}} labelStyle={{color:DIM}} formatter={(v,_,p)=>[p.payload.full.toLocaleString("nl-NL"),"NAV"]}/>
...
function PnLTrajectory({data,enriched,sym}) {
  ...
  const chartData = (data.history||[]).map(h=>({
    date:  h.date,
    label: fmtDateChart(h.date),
    total:      h.total_pnl ?? (h.nav - (h.deposits_total ?? totalDeposited)),
    unrealized: h.unrealized ?? 0,
    realized:   h.realized   ?? 0,
  }));

  // Append current live values as "today" if not already saved today
  const today = new Date().toISOString().slice(0,10);
  const lastH = chartData[chartData.length-1];
  if (!lastH || lastH.date !== today) {
    const totalU = enriched.reduce((s,p)=>s+p.upnl,0);
    const totalR = (data.realized||[]).reduce((s,r)=>s+r.pnl,0);
    const totalP = data.nav.total - totalDeposited;
    chartData.push({date:today,label:fmtDateChart(today)+"*",total:totalP,unrealized:totalU,realized:totalR});
  }
  ...
  const dd      = current.total - athVal;
  const ddPct   = athVal>0?(dd/athVal*100):0;

  const minDate = chartData[0]?.date||"";
  const maxDate = chartData[chartData.length-1]?.date||"";

  // Key events from deposits + realized trades
  const events = [
    ...(data.deposits||[]).map(d=>({date:d.date,label:`+${sym}${d.amount.toLocaleString("nl-NL")} ${d.desc||"Storting"}`,color:BLUE,dot:"💰"})),
    ...(data.realized||[]).filter(r=>Math.abs(r.pnl)>1000).map(r=>({date:null,label:`${r.sym}: ${r.pnl>=0?"+":""}${sym}${Math.round(r.pnl).toLocaleString("nl-NL")} · ${r.desc}`,color:r.pnl>=0?GREEN:RED,dot:r.pnl>=0?"▲":"▼"})),
    athIdx>=0?{date:chartData[athIdx].date,label:`ATH ${sym}${Math.round(athVal).toLocaleString("nl-NL")}`,color:GREEN,dot:"🏆"}:null,
  ].filter(Boolean).sort((a,b)=>(a.date||"z").localeCompare(b.date||"z"));

  // Deposit reference lines
  const depositLines = showDeposits
    ? (data.deposits||[]).filter(d=>chartData.some(c=>c.date>=d.date))
    : [];

  const customTooltip = ({active,payload,label}) => {
    if (!active||!payload?.length) return null;
    return (
      <div style={{background:"#1a1c22",border:`1px solid ${LINE}`,borderRadius:8,padding:"10px 14px",fontFamily:MONO,fontSize:12}}>
        <div style={{color:DIM,marginBottom:6}}>{label}</div>
        {payload.map((p,i)=>(
          <div key={i} style={{color:p.color||"#fff",marginBottom:2}}>
            {p.name}: {sym}{Math.round(p.value).toLocaleString("nl-NL")}
          </div>
        ))}
      </div>
    );
  ...
  const yFormatter = v => {
    ...
    return (v<0?"-":"")+sym+(abs>=1000?Math.round(abs/1000)+"K":Math.round(abs));
  };
      ...
      <div style={{...sCard,padding:"22px 24px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:12,marginBottom:18}}>
          <div>
            ...
            <div style={{fontSize:40,fontWeight:800,color:cof(current.total),letterSpacing:"-.03em",lineHeight:1,fontFamily:MONO}}>
              {fmtS(sym,current.total)}
            </div>
            <div style={{fontSize:12,color:DIM,marginTop:6}}>
              Total P&L as of today · op {sym}{totalDeposited.toLocaleString("nl-NL")} kapitaal
            </div>
          </div>
          <div style={{display:"flex",gap:24}}>
            <div style={{textAlign:"right"}}>
              <div style={{fontSize:10,color:DIM,textTransform:"uppercase",letterSpacing:".1em",marginBottom:4}}>ATH</div>
              <div style={{fontFamily:MONO,fontSize:20,fontWeight:700,color:GREEN}}>{sym}{Math.round(athVal).toLocaleString("nl-NL")}</div>
        ...
        <div style={{display:"flex",gap:8,marginBottom:16}}>
          <button onClick={()=>setShowDeposits(!showDeposits)} style={{
            background:showDeposits?"rgba(91,156,246,.15)":"rgba(255,255,255,.05)",
            border:`1px solid ${showDeposits?BLUE:LINE}`,borderRadius:6,
            padding:"5px 14px",color:showDeposits?BLUE:DIM,fontSize:11,fontWeight:700,
            cursor:"pointer",fontFamily:SANS,textTransform:"uppercase",letterSpacing:".08em"
          }}>+ SHOW DEPOSITS</button>
        </div>

        {/* Legend */}
        <div style={{display:"flex",gap:20,marginBottom:14,flexWrap:"wrap"}}>
          {[
            {color:GOLD,   dash:true, label:"Realized P&L · gesloten trades"},
            {color:PURPLE, dash:false,label:"Unrealized P&L · open posities"},
            {color:GREEN,  dash:false,label:"Total P&L · NAV − Stortingen"},
          ].map((l,i)=>(
        ...
        <ResponsiveContainer width="100%" height={300}>
          <ComposedChart data={chartData} margin={{top:10,right:10,left:0,bottom:0}}>
            ...
            <YAxis tick={{fill:DIM,fontSize:10,fontFamily:MONO}} axisLine={false} tickLine={false} tickFormatter={yFormatter} width={56}/>
            <Tooltip content={customTooltip}/>
            <ReferenceLine y={0} stroke="rgba(255,255,255,.18)" strokeWidth={1}/>
            {depositLines.map((d,i)=>(
              <ReferenceLine key={i} x={fmtDateChart(d.date)} stroke={`${BLUE}50`} strokeDasharray="4 2"
                label={{value:`+${sym}${(d.amount/1000).toFixed(0)}K`,position:"top",fill:BLUE,fontSize:9,fontFamily:MONO}}/>
            ))}
            {/* ATH dot */}
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:10}}>
            {events.map((e,i)=>(
              <div key={i} style={{display:"flex",gap:10,alignItems:"flex-start"}}>
                ...
                <div>
                  {e.date&&<div style={{fontFamily:MONO,fontSize:10,color:DIM,marginBottom:2}}>{fmtDateShort(e.date)}</div>}
                  <div style={{fontSize:12,color:"rgba(255,255,255,.7)"}}>{e.label}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {chartData.length<3&&(
        <div style={{...sCard,padding:"20px",textAlign:"center"}}>
          <div style={{fontSize:12,color:DIM,fontStyle:"italic"}}>
            Sla dagelijks updates op om de P&L trajectory op te bouwen.
            Voeg ook je stortingen toe via <strong style={{color:GOLD}}>✏ Update → Stortingen</strong>.
          </div>
        </div>
      )}
    </div>
...
function OverviewTab({data,enriched,lastPx,sym}) {
  const totalUpnl=enriched.reduce((s,p)=>s+p.upnl,0);
  const totalReal=data.realized.reduce((s,r)=>s+r.pnl,0);
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
        ...
        <MetCard lbl="Aandelen" val={fmtM(sym,enriched.reduce((s,p)=>s+p.value,0))} sub={enriched.length+" posities"} vc={GOLD}/>
        <MetCard lbl="Cash" val={fmtM(sym,data.nav.cash)} sub={(data.nav.total>0?(data.nav.cash/data.nav.total*100):0).toFixed(1)+"% van NAV"} vc={BLUE}/>
        <MetCard lbl="Unrealized P&L" val={fmtS(sym,totalUpnl)} sub={fmtP(totalCost>0?totalUpnl/totalCost*100:0)+" op kostprijs"} vc={cof(totalUpnl)} accent/>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
        <EarningsCalendar positions={data.positions}/>
        <div style={{...sCard,padding:"16px 18px"}}>
          <div style={{fontSize:10,color:DIM,textTransform:"uppercase",letterSpacing:".12em",marginBottom:12}}>
            {topMovers.length?"Beweging vs vorige update":"Movers"}
          </div>
          ...
          )):<div style={{fontSize:12,color:DIM,fontStyle:"italic"}}>Sla meerdere updates op om beweging te zien</div>}
        </div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
        <div style={{...sCard,padding:"16px 18px"}}>
        ...
        <div style={{...sCard,padding:"16px 18px"}}>
          <div style={{fontSize:10,color:DIM,textTransform:"uppercase",letterSpacing:".12em",marginBottom:12}}>Gerealiseerd · netto {fmtS(sym,totalReal)}</div>
...
function PosTab({data,enriched,lastPx,sym}) {
  ...
  return (
    <div>
      <div style={{...sCard,padding:"16px 18px",marginBottom:14}}>
        <div style={{fontSize:10,color:DIM,textTransform:"uppercase",letterSpacing:".12em",marginBottom:10}}>Allocatie</div>
      ...
      <div style={{...sCard,overflow:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse"}}>
          <thead>
            <tr style={{borderBottom:`1px solid ${LINE}`}}>
              {["Sym","Koers (native)","Δ update","Waarde","P&L","% gain","PT","Upside","Earnings","% NAV"].map((h,i)=>(
          ...
          <tbody>
            {enriched.map((p,i)=>{
              ...
              const upside=ptBase>0?((ptBase-p.pxEUR)/p.pxEUR*100):null;
              const days=daysUntil(p.earnings);
              const isExp=exp===p.id;
              return (
                <>
                  <tr key={p.id} onClick={()=>setExp(isExp?null:p.id)}
                    style={{borderBottom:isExp?`1px solid ${GOLD}40`:i<enriched.length-1?`1px solid ${LINE}`:"none",cursor:"pointer"}}
                    ...
                    <td style={{padding:"10px 10px",textAlign:"right"}}>
                      <PriceDisplay px={p.px} ccy={p.ccy} pxEUR={p.pxEUR} sym={sym}/>
                    </td>
                    ...
                    <tr key={`x${p.id}`} style={{borderBottom:`1px solid ${LINE}`,background:"rgba(212,168,83,.03)"}}>
                      <td colSpan={10} style={{padding:"10px 14px"}}>
                        <div style={{display:"flex",gap:24,flexWrap:"wrap",fontSize:11}}>
                          ...
                          <span style={{color:DIM}}>Kostprijs: <span style={{color:"#e8e8ea",fontFamily:MONO}}>{fmtM(sym,p.cost)}</span></span>
                          <span style={{color:DIM}}>Thesis: <span style={{color:p.thesis?"#e8e8ea":DIM,fontStyle:p.thesis?"normal":"italic"}}>{p.thesis||"Geen notitie"}</span></span>
                        </div>
                      </td>
                    </tr>
                  )}
          ...
          <tfoot>
            <tr style={{borderTop:`1px solid ${LINE}`}}>
              <td style={{padding:"10px 10px",fontWeight:700}}>Totaal</td><td/><td/>
        ...
        <div style={{padding:"8px 14px",fontSize:10,color:DIM,borderTop:`1px solid ${LINE}`}}>Klik op een rij voor details &amp; thesis</div>
      </div>
    </div>
  );
}

function PerfTab({data,enriched,sym}) {
  const totalUpnl=enriched.reduce((s,p)=>s+p.upnl,0);
  const totalReal=data.realized.reduce((s,r)=>s+r.pnl,0);
  const navDelta=data.nav.total-data.nav.prior;
  const top3=enriched.slice(0,3).reduce((s,p)=>s+p.value,0);

  return (
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <div style={{...sCard,padding:"20px 22px"}}>
        <div style={{fontSize:10,color:DIM,textTransform:"uppercase",letterSpacing:".12em",marginBottom:14}}>NAV Historiek</div>
        <NavChart history={data.history}/>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:0,...sCard,padding:"18px 22px"}}>
        {[
          {lbl:"NAV vandaag",v:fmtM(sym,data.nav.total),c:"#f9fafb"},
          {lbl:"Dag change",v:fmtS(sym,navDelta),c:cof(navDelta)},
          {lbl:"Unrealized",v:fmtS(sym,totalUpnl),c:cof(totalUpnl)},
          {lbl:"Gerealiseerd",v:fmtS(sym,totalReal),c:cof(totalReal)},
          {lbl:"TWR vandaag",v:"+"+data.twr.toFixed(2)+"%",c:GREEN},
        ].map((m,i)=>(
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
        <div style={{...sCard,padding:"16px 18px"}}>
          <div style={{fontSize:10,color:DIM,textTransform:"uppercase",letterSpacing:".12em",marginBottom:12}}>Concentratie</div>
          {[["#1 positie",enriched[0]?.value??0,enriched[0]?.sym??"—"],["Top 3",top3,"van NAV"],["Top 5",enriched.slice(0,5).reduce((s,p)=>s+p.value,0),"van NAV"]].map(([lbl,v,extra],i)=>{
        ...
        <div style={{...sCard,padding:"16px 18px"}}>
          <div style={{fontSize:10,color:DIM,textTransform:"uppercase",letterSpacing:".12em",marginBottom:12}}>PT vs huidige koers</div>
          {enriched.filter(p=>p.pt>0).slice(0,6).map((p,i,arr)=>{
            const ptBase=r2(p.pt*fxOf(p.ccy,data.fx));
            const upside=((ptBase-p.pxEUR)/p.pxEUR*100);
            return (
              <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 0",borderBottom:i<arr.length-1?`1px solid ${LINE}`:"none",fontSize:12}}>
                ...
                <div style={{flex:1,height:4,background:"rgba(255,255,255,.06)",borderRadius:2,overflow:"hidden",margin:"0 10px"}}>
                  <div style={{width:`${Math.min(100,Math.max(0,(p.pxEUR/ptBase)*100))}%`,height:"100%",background:cof(upside),borderRadius:2}}/>
                </div>
                <Pill v={upside}/>
              </div>
            );
          })}
          {!enriched.filter(p=>p.pt>0).length&&<div style={{fontSize:12,color:DIM,fontStyle:"italic"}}>Voeg PT toe via edit-modus</div>}
        </div>
      </div>
    </div>
  );
...
function RealizedTab({data,sym}) {
  ...
  const total=data.realized.reduce((s,r)=>s+r.pnl,0);
  const wins=data.realized.filter(r=>r.pnl>0).reduce((s,r)=>s+r.pnl,0);
  const losses=data.realized.filter(r=>r.pnl<0).reduce((s,r)=>s+r.pnl,0);
  return (
    <div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:14}}>
        <MetCard lbl="Netto gerealiseerd" val={fmtS(sym,total)} vc={cof(total)}/>
        <MetCard lbl="Winsten" val={fmtS(sym,wins)} vc={GREEN}/>
        <MetCard lbl="Verliezen" val={fmtS(sym,losses)} vc={RED}/>
      </div>
...
function WatchlistTab({data,sym}) {
  if (!data.watchlist?.length) return (
    <div style={{...sCard,padding:"40px",textAlign:"center"}}>
      <div style={{color:DIM,fontSize:12,fontStyle:"italic"}}>Voeg aandelen toe via de edit-modus</div>
    </div>
  );
  return (
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      {data.watchlist.map(w=>{
        const pxBase=r2(w.px*fxOf(w.ccy,data.fx));
        const ptBase=w.pt>0?r2(w.pt*fxOf(w.ccy,data.fx)):0;
        const upside=ptBase>0?((ptBase-pxBase)/pxBase*100):null;
        return (
          <div key={w.id} style={{...sCard,padding:"18px 20px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:w.thesis?10:0}}>
              ...
              <div style={{display:"flex",gap:12,alignItems:"center"}}>
                <div style={{textAlign:"right"}}>
                  <div style={{fontSize:10,color:DIM,textTransform:"uppercase",letterSpacing:".08em"}}>Koers</div>
                  <div style={{fontFamily:MONO,fontSize:14,fontWeight:600}}><PriceDisplay px={w.px} ccy={w.ccy} pxEUR={pxBase} sym={sym}/></div>
                </div>
                {ptBase>0&&<div style={{textAlign:"right"}}><div style={{fontSize:10,color:DIM,textTransform:"uppercase",letterSpacing:".08em"}}>PT</div><div style={{fontFamily:MONO,fontSize:14,fontWeight:600}}>{sym}{ptBase.toFixed(2)}</div></div>}
...
function EditPrices({draft,setDraft,sym,onRefresh,refreshState}) {
    ...
    <div>
      ...
      <div style={{...sCard,padding:"14px 18px",marginBottom:18,display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
        <div style={{flex:1,minWidth:200}}>
          <div style={{fontSize:10,color:DIM,textTransform:"uppercase",letterSpacing:".1em",marginBottom:4}}>Automatische koersupdate</div>
          <div style={{fontSize:12,color:"rgba(255,255,255,.5)"}}>
            {hasKey
              ? `API key aanwezig · ${refreshState==="loading"?"Laden…":refreshState==="ok"?"Koersen bijgewerkt ✓":refreshState==="error"?"Fout bij ophalen":"Klik Refresh"}`
              : "Voeg Twelve Data API key toe in Instellingen voor auto-refresh"}
          </div>
        </div>
        {draft.last_price_update&&(
          <div style={{fontSize:11,color:DIM,fontFamily:MONO}}>
            Bijgewerkt: {new Date(draft.last_price_update).toLocaleTimeString("nl-NL",{hour:"2-digit",minute:"2-digit"})}
          </div>
        )}
        <button onClick={onRefresh} disabled={!hasKey||refreshState==="loading"} style={{
          ...sBtn(hasKey&&refreshState!=="loading"),
          opacity:(!hasKey||refreshState==="loading")?0.5:1,
          display:"flex",alignItems:"center",gap:6,
        }}>
          <span style={{display:"inline-block",animation:refreshState==="loading"?"spin 1s linear infinite":"none"}}>↺</span>
          {refreshState==="loading"?"Laden…":"Refresh Koersen"}
        </button>
      </div>

      {/* Meta fields */}
      <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr 1fr",gap:12,marginBottom:20,alignItems:"end"}}>
        <div><label style={sLabel}>Snapshot datum</label><input value={draft.snapshot} onChange={e=>upd("snapshot",e.target.value)} style={sIn}/></div>
        {[["NAV totaal",draft.nav.total,v=>updNav("total",v)],["Prior NAV",draft.nav.prior,v=>updNav("prior",v)],["Cash",draft.nav.cash,v=>updNav("cash",v)]].map(([l,v,fn],i)=>(
          <div key={i}><label style={sLabel}>{l} ({sym})</label><input type="number" step="1" value={v} onChange={e=>fn(e.target.value)} style={sIn}/></div>
        ))}
        <div><label style={sLabel}>EUR/USD</label><input type="number" step="0.00001" value={draft.fx.usd} onChange={e=>updFx("usd",e.target.value)} style={sIn}/></div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 3fr",gap:12,marginBottom:20,maxWidth:400}}>
        <div><label style={sLabel}>TWR %</label><input type="number" step="0.01" value={draft.twr} onChange={e=>setDraft(d=>({...d,twr:parseFloat(e.target.value)||0}))} style={sIn}/></div>
      </div>

      {/* Price table */}
      <div style={{fontSize:10,color:GOLD,textTransform:"uppercase",letterSpacing:".12em",marginBottom:10}}>
        Koersen (native valuta) · EUR equivalent automatisch berekend
      </div>
      <div style={{...sCard,overflow:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse"}}>
          <thead>
            <tr style={{borderBottom:`1px solid ${LINE}`}}>
              {["Sym","Ccy","Koers ↗","EUR equiv.","Waarde","Unrealized P&L","Earnings","PT"].map((h,i)=>(
              ...
              return (
                <tr key={p.id} style={{borderBottom:`1px solid rgba(255,255,255,.03)`}}>
                  ...
                  <td style={{padding:"7px 10px"}}>
                    <span style={{fontSize:10,padding:"2px 6px",borderRadius:6,fontFamily:MONO,fontWeight:700,background:p.ccy==="USD"?"rgba(91,156,246,.15)":p.ccy==="SEK"?"rgba(212,168,83,.15)":"rgba(62,207,142,.1)",color:p.ccy==="USD"?BLUE:p.ccy==="SEK"?GOLD:GREEN}}>{p.ccy}</span>
                  </td>
                  <td style={{padding:"6px 10px",textAlign:"right"}}>
                    <input type="number" step="0.01" value={p.px} onChange={e=>updPx(p.id,e.target.value)} style={{...sInHL,width:100,textAlign:"right"}}/>
                  </td>
                  <td style={{padding:"7px 10px",textAlign:"right",fontFamily:MONO,fontSize:12,color:DIM}}>
                    {p.ccy!=="EUR"&&<span>{sym}{p.pxEUR.toFixed(2)}</span>}
                  </td>
                  ...
                  <td style={{padding:"7px 10px",textAlign:"right",fontFamily:MONO,fontSize:11,color:ecColor(days)||DIM}}>
                    {p.earnings?`${fmtEarnings(p.earnings)} (${days}d)`:"—"}
                  </td>
                  <td style={{padding:"7px 10px",textAlign:"right",fontFamily:MONO,fontSize:11,color:DIM}}>
                    {p.pt>0?p.pt:"—"}
                  </td>
                </tr>
              );
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <div style={{fontSize:11,color:DIM}}>Posities beheren</div>
        <button onClick={add} style={{...sBtn(true),padding:"7px 14px",fontSize:11}}>+ Positie</button>
      </div>
      <div style={{...sCard,overflow:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse"}}>
          <thead><tr style={{borderBottom:`1px solid ${LINE}`}}>
            {["Sym","Naam","Qty","Koers","Ccy",`Kostprijs (${sym})`,"Earnings","PT","TD Sym",""].map((h,i)=>(
              ...
              return (
                <>
                  <tr key={p.id} style={{borderBottom:isE?`1px solid ${GOLD}40`:`1px solid ${LINE}`}}>
                    <td style={{padding:"6px 8px"}}><input value={p.sym} onChange={e=>upd(p.id,"sym",e.target.value.toUpperCase())} style={{...sIn,width:70}} placeholder="SYM"/></td>
                    <td style={{padding:"6px 8px"}}><input value={p.name} onChange={e=>upd(p.id,"name",e.target.value)} style={{...sIn,width:140}} placeholder="Naam"/></td>
                    ...
                    <td style={{padding:"6px 8px",textAlign:"right"}}><input type="number" step="0.01" value={p.pt} placeholder="PT" onChange={e=>upd(p.id,"pt",e.target.value)} style={{...sIn,width:70,textAlign:"right"}}/></td>
                    <td style={{padding:"6px 8px"}}><input value={p.td_sym||""} onChange={e=>upd(p.id,"td_sym",e.target.value)} style={{...sIn,width:100}} placeholder="bijv. SIVE:XSTO"/></td>
                  ...
                  {isE&&(
                    <tr key={`x${p.id}`} style={{borderBottom:`1px solid ${LINE}`,background:"rgba(212,168,83,.03)"}}>
                      <td colSpan={10} style={{padding:"10px 10px"}}>
                        <label style={sLabel}>Thesis</label>
                        <textarea value={p.thesis} onChange={e=>upd(p.id,"thesis",e.target.value)} placeholder="Beleggingsthesis, exit-targets..." style={{...sIn,height:60,resize:"vertical"}}/>
                      </td>
                    </tr>
                  )}
                </>
        ...
        <div style={{padding:"8px 14px",fontSize:10,color:DIM,borderTop:`1px solid ${LINE}`}}>
          "TD Sym" = Twelve Data symbool override voor niet-US aandelen (bijv. <code style={{background:"rgba(255,255,255,.06)",padding:"1px 5px",borderRadius:4}}>SIVE:XSTO</code>)
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:14}}>
        <div style={{fontSize:11,color:DIM}}>Gesloten posities</div>
        <button onClick={add} style={{...sBtn(true),padding:"7px 14px",fontSize:11}}>+ Trade</button>
      </div>
      <div style={{...sCard,padding:"8px 0"}}>
        {draft.realized.map((r,i)=>(
          <div key={r.id} style={{display:"flex",gap:10,alignItems:"center",padding:"6px 14px",borderBottom:i<draft.realized.length-1?`1px solid ${LINE}`:"none"}}>
            ...
            <input value={r.desc} onChange={e=>upd(r.id,"desc",e.target.value)} style={{...sIn,flex:1}} placeholder="Beschrijving"/>
...
function EditDeposits({draft,setDraft,sym}) {
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <div>
          <div style={{fontSize:11,color:DIM}}>Kapitaalstortingen</div>
          <div style={{fontSize:12,color:GOLD,fontFamily:MONO,marginTop:4}}>Totaal: {sym}{total.toLocaleString("nl-NL")}</div>
        </div>
        <button onClick={add} style={{...sBtn(true),padding:"7px 14px",fontSize:11}}>+ Storting</button>
      </div>
      <div style={{...sCard,fontSize:13}}>
        {!(draft.deposits?.length)
          ? <div style={{padding:"24px",textAlign:"center",color:DIM,fontStyle:"italic"}}>Nog geen stortingen — voeg toe voor de P&L Trajectory grafiek</div>
          : [...(draft.deposits||[])].sort((a,b)=>a.date.localeCompare(b.date)).map((d,i,arr)=>(
            <div key={d.id} style={{display:"flex",gap:10,alignItems:"center",padding:"8px 14px",borderBottom:i<arr.length-1?`1px solid ${LINE}`:"none"}}>
              ...
              <input type="number" step="100" value={d.amount} placeholder="Bedrag" onChange={e=>upd(d.id,"amount",e.target.value)} style={{...sInHL,width:110,textAlign:"right"}}/>
              <input value={d.desc} onChange={e=>upd(d.id,"desc",e.target.value)} style={{...sIn,flex:1}} placeholder="Beschrijving (optioneel)"/>
...
function EditWatchlist({draft,setDraft}) {
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:14}}>
        <div style={{fontSize:11,color:DIM}}>Watchlist</div>
        <button onClick={add} style={{...sBtn(true),padding:"7px 14px",fontSize:11}}>+ Toevoegen</button>
      </div>
          ...
          return (
            <div key={w.id} style={{...sCard,padding:"12px 16px"}}>
              <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:isE?12:0}}>
                ...
                <input value={w.name} onChange={e=>upd(w.id,"name",e.target.value)} style={{...sIn,flex:1}} placeholder="Naam"/>
                <input type="number" step="0.01" value={w.px} onChange={e=>upd(w.id,"px",e.target.value)} style={{...sInHL,width:90,textAlign:"right"}} placeholder="Koers"/>
                <select value={w.ccy} onChange={e=>upd(w.id,"ccy",e.target.value)} style={{...sIn,width:65,background:"rgba(255,255,255,.06)"}}>
                  {["USD","EUR","SEK","GBP"].map(c=><option key={c} value={c}>{c}</option>)}
                </select>
                <input type="number" step="0.01" value={w.pt} onChange={e=>upd(w.id,"pt",e.target.value)} style={{...sIn,width:80,textAlign:"right"}} placeholder="PT"/>
              ...
              {isE&&<><label style={sLabel}>Thesis</label><textarea value={w.thesis} onChange={e=>upd(w.id,"thesis",e.target.value)} placeholder="Waarom staat dit op de watchlist?" style={{...sIn,height:56,resize:"vertical"}}/></>}
            </div>
          );
        })}
      </div>
...
function EditSettings({draft,setDraft}) {
  ...
  return (
    <div style={{maxWidth:500,display:"flex",flexDirection:"column",gap:14}}>
      <div style={{...sCard,padding:"20px 22px"}}>
        <div style={{fontSize:12,color:DIM,marginBottom:16,fontWeight:600}}>Valuta</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
          <div><label style={sLabel}>Basisvaluta (bijv. EUR)</label><input value={draft.currency?.base||"EUR"} onChange={e=>setDraft(d=>({...d,currency:{...d.currency,base:e.target.value.toUpperCase()}}))} style={sIn}/></div>
          <div><label style={sLabel}>Symbool (bijv. €)</label><input value={draft.currency?.symbol||"€"} onChange={e=>setDraft(d=>({...d,currency:{...d.currency,symbol:e.target.value}}))} style={sIn}/></div>
        </div>
      </div>
      <div style={{...sCard,padding:"20px 22px"}}>
        <div style={{fontSize:12,color:DIM,marginBottom:4,fontWeight:600}}>Twelve Data API Key</div>
        <div style={{fontSize:11,color:DIM,marginBottom:14,lineHeight:1.6}}>
          Gratis account op <strong style={{color:"rgba(255,255,255,.6)"}}>twelvedata.com</strong> → Dashboard → API Key kopiëren.
          Gratis tier: 800 credits/dag (voldoende voor ~50 refreshes/dag).
          De API key wordt lokaal opgeslagen in je browser.
        </div>
        <div><label style={sLabel}>API Key</label>
          <input type="password" value={draft.api_key||""} onChange={e=>setDraft(d=>({...d,api_key:e.target.value}))} style={sIn} placeholder="Plak hier je Twelve Data API key"/>
        </div>
        {draft.api_key&&(
          <div style={{marginTop:10,fontSize:11,color:GREEN}}>✓ API key aanwezig ({draft.api_key.length} tekens)</div>
        )}
        <div style={{marginTop:14,fontSize:11,color:DIM,lineHeight:1.6}}>
          Niet-US aandelen vereisen een exchange-suffix in Twelve Data.
          Stel dit in per positie via <strong style={{color:"rgba(255,255,255,.5)"}}>Edit → Posities → kolom "TD Sym"</strong>.<br/>
          Voorbeelden: <code style={{background:"rgba(255,255,255,.06)",padding:"1px 4px",borderRadius:3}}>SIVE:XSTO</code>, <code style={{background:"rgba(255,255,255,.06)",padding:"1px 4px",borderRadius:3}}>VWCE:XETRA</code>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const [editTab,      setEditTab]      = useState("koersen");
  ...
  const handleRefresh = useCallback(async () => {
    ...
    setRefreshErr("");
    try {
      // Build symbol map including custom td_sym overrides
      const positionsWithMap = draft.positions.map(p => ({...p, _tdSym: p.td_sym?.trim() || TD_MAP[p.sym] || p.sym}));
      const {prices, fx} = await fetchPrices(draft.api_key, positionsWithMap, draft.currency);
      ...
      setDraft(d => ({
        ...
        positions: d.positions.map(p => {
          ...
          const newPx = prices[p.sym] ?? prices[tdSym];
  ...
  function save() {
    ...
    const totalPnl     = draft.nav.total - totalDep;
    const posSnap      = {};
    draft.positions.forEach(p=>{ posSnap[p.sym]=p.px; });

    if (!last || last.date !== today) {
      newDraft.history = [...hist, {
        date: today, nav: draft.nav.total,
        unrealized: totalU, realized: totalR,
        deposits_total: totalDep, total_pnl: totalPnl,
        px: posSnap,
      }].slice(-365);
    }
  ...
  function reset() {
    if(!confirm("Verwijder alle data en start opnieuw?")) return;
    localStorage.removeItem(STORAGE_KEY); setData(EXAMPLE); setView("dashboard");
  }

  if(view==="loading") return <div style={{background:BG,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:SANS,color:DIM}}>Laden…</div>;
  ...
  const DTABS = ["overview","posities","p&l","performance","gerealiseerd","watchlist"];
  const ETABS = ["koersen","posities","gerealiseerd","stortingen","watchlist","instellingen"];
  const DLABELS = {overview:"Overview","p&l":"P&L Chart",posities:"Posities",performance:"Performance",gerealiseerd:"Gerealiseerd",watchlist:"Watchlist"};
  const ELABELS = {koersen:"Koersen",posities:"Posities",gerealiseerd:"Gerealiseerd",stortingen:"Stortingen",watchlist:"Watchlist",instellingen:"Instellingen"};
  ...
  return (
    <div style={{background:BG,minHeight:"100vh",color:"#e8e8ea",fontFamily:SANS}}>
      ...
      <div style={{padding:"26px 28px 0",borderBottom:`1px solid ${LINE}`,position:"sticky",top:0,background:BG,zIndex:20}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
          <div>
            ...
            <div style={{fontSize:isEdit?26:38,fontWeight:800,letterSpacing:"-.03em",lineHeight:1,fontFamily:isEdit?SANS:MONO}}>
              {isEdit?"Dagelijkse update":fmtM(sym,data.nav.total)}
            </div>
            {!isEdit&&(
          ...
          <div style={{display:"flex",gap:8,paddingTop:4,flexWrap:"wrap",justifyContent:"flex-end",alignItems:"flex-start"}}>
            {isEdit?(
              <>
                <button onClick={()=>setView("dashboard")} style={sBtn(false)}>Annuleren</button>
                <button onClick={save} style={{...sBtn(true),boxShadow:`0 0 18px rgba(212,168,83,.25)`}}>Opslaan</button>
              </>
            ):(
              <>
                {saved&&<span style={{fontFamily:MONO,fontSize:12,color:GREEN,alignSelf:"center"}}>✓ Opgeslagen</span>}
                <button onClick={()=>importRef.current?.click()} style={{...sBtn(false),fontSize:11}}>↑ Import</button>
                <button onClick={exportData} style={{...sBtn(false),fontSize:11}}>↓ Export</button>
        ...
        {!isEdit&&(
          <div style={{display:"flex",gap:14,padding:"8px 0 14px",fontSize:11,color:DIM,flexWrap:"wrap",alignItems:"center"}}>
            ...
            <span>{enriched.length} posities · {(data.history||[]).length} snapshots</span>
            {data.last_price_update&&<><span style={{opacity:.3}}>·</span><span>Koersen: {new Date(data.last_price_update).toLocaleTimeString("nl-NL",{hour:"2-digit",minute:"2-digit"})}</span></>}
          </div>
        )}

        {refreshErr&&isEdit&&(
          <div style={{background:"rgba(240,96,112,.08)",border:`1px solid rgba(240,96,112,.2)`,borderRadius:8,padding:"8px 14px",marginBottom:8,fontSize:12,color:RED}}>
            ⚠ {refreshErr}
          </div>
        )}
        ...
        <div style={{display:"flex",marginTop:isEdit?10:0,overflowX:"auto"}}>
          {(isEdit?ETABS:DTABS).map(t=>{
            ...
            const label=(isEdit?ELABELS:DLABELS)[t]||t;
            return <button key={t} style={sTab(active)} onClick={()=>isEdit?setEditTab(t):setTab(t)}>{label}</button>;
          })}
        </div>
      </div>
      ...
      <div style={{padding:"22px 28px 56px",maxWidth:1150,animation:"fadein .2s ease"}}>
        {isEdit?(
          <>
            {editTab==="koersen"      && <EditPrices    draft={draft} setDraft={setDraft} sym={draft.currency?.symbol||"€"} onRefresh={handleRefresh} refreshState={refreshState}/>}
            {editTab==="posities"     && <EditPositions  draft={draft} setDraft={setDraft} sym={draft.currency?.symbol||"€"}/>}
            {editTab==="gerealiseerd" && <EditRealized   draft={draft} setDraft={setDraft} sym={draft.currency?.symbol||"€"}/>}
            {editTab==="stortingen"   && <EditDeposits   draft={draft} setDraft={setDraft} sym={draft.currency?.symbol||"€"}/>}
            {editTab==="watchlist"    && <EditWatchlist  draft={draft} setDraft={setDraft}/>}
            {editTab==="instellingen" && <EditSettings   draft={draft} setDraft={setDraft}/>}
          </>
        ):(
          <>
            {tab==="overview"     && <OverviewTab    data={data} enriched={enriched} lastPx={lastPx} sym={sym}/>}
            {tab==="posities"     && <PosTab         data={data} enriched={enriched} lastPx={lastPx} sym={sym}/>}
            {tab==="p&l"          && <PnLTrajectory  data={data} enriched={enriched} sym={sym}/>}
            {tab==="performance"  && <PerfTab        data={data} enriched={enriched} sym={sym}/>}
            {tab==="gerealiseerd" && <RealizedTab    data={data} sym={sym}/>}
